...
contract SentinelsBridge {
    ...
    function SignVerify(string memory message, bytes memory signature) public view returns (bool) {
        // bytes memory msgs = bytes(message);
        // // address reco = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n", Strings.toString(msgs.length), msgs)).recover(signature);
        // address reco = msgs.toEthSignedMessageHash().recover(signature);
        // console.log(reco);
        // return reco == msg.sender;
